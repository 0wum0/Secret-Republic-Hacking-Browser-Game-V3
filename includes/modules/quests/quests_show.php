    ...
    if ($_POST["play"] && $user["questManager"]) {
      ...
      if ($GET["group"])
	  {
        ...
        if ($GET["mission"])
		{
          ...
          $db->groupBy('q.id, q.title, q.summary, q.time, q.level, q.type, party, q.qgroup_id, qgroup_order');
          $db->orderBy('qgroup_order', 'desc');
          $quests = $db->get('quests q', null, 'q.id, q.title, q.summary, q.time, q.id, q.level, MAX(qu.last_done) as done, q.type, MAX(qur.quest) as qdone, party, q.qgroup_id');
